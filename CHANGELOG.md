...
- Bump package version for the coordinated LDGR 0.1.3 release train.
- Refresh docs/help around global prompt installation, prompt composition, and removal of bundle prompt sources.
...
- Align adapter UX with the conduct-style pattern: `ldgr-research install` installs the adapter bundle plus harness resources, `ldgr-research init` ensures the global research loop prompt exists, and docs prefer canonical `ldgr research <command>` dispatch.
...
- Bump package version for the coordinated LDGR 0.1.2 release train.
- Coordinate binary release publishing from the root LDGR integration checkout.
